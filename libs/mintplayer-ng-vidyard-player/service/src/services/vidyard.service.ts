...
export class VidyardService implements IApiService {
  ...
  public createPlayer(options: PlayerOptions, componentDestroy: Subject<boolean>) : Promise<PlayerAdapter> {
    return new Promise((resolvePlayer, rejectPlayer) => {
      ...
      adapter$.pipe(filter(a => !!a), map(a => a!), take(1), takeUntil(destroyRef), takeUntil(componentDestroy))
      ...
      playerReady$.pipe(filter(([ready]) => ready), take(1), takeUntil(destroyRef), takeUntil(componentDestroy))
        .subscribe(([_, plr]) => {
          const player = plr!;
