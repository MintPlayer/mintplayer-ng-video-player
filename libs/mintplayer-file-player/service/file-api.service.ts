...
export class FileApiService implements IApiService {
            ...
            fromEvent(mediaElement, 'canplay')
                ...
                adapter = createPlayerAdapter({
                    ...
                    setFullscreen: (isFullscreen) => {
                        if (isFullscreen) {
                            ...
                            if ((typeof window !== 'undefined') && (document.fullscreenElement === divElement)) {
                    ...
                    setPip: (isPip) => {
                        if (mediaElement instanceof HTMLVideoElement) {
                            if (isPip) {
                                ...
                                if ((typeof window !== 'undefined') && (document.pictureInPictureElement === mediaElement)) {
                ...
                if (mediaElement instanceof HTMLAudioElement) {
                    ...
                    fromEvent(divElement, 'fullscreenchange')
                        .pipe(filter(_ => typeof window !== 'undefined'), takeUntil(destroyRef), takeUntil(destroy))
    ...
    private frameLooper(analyser: AnalyserNode, canvas: HTMLCanvasElement, destroy: Subject<boolean>) {
        ...
        let cancelled = false;
        destroy.pipe(take(1)).subscribe(() => cancelled = true);
